...
### =======================
sim_hetero <- function(i_sim, field, field_raw, field_col, exp_design, coef_data, correlation_rho) {
  ...
  #' 
  #' # test <-
  #' #   data_with_N[, .(cell_id, plot_id, N)] %>%
  #' #   left_join(field[,'cell_id'], ., by='cell_id')
  #' 
  #' # #=== N mapping ===#
  #' # ggplot() +
  #' #   geom_sf(data = test, aes(fill = N), size=0.1) +
  #' #   ggtitle(temp_design$cell_columns_in_plot) +
  ...
  #'     )] %>%
  #'     setnames(c("yield_accuracy", "pCorn", "pN", "opt_N_gwr", "pi_gwr","opt_N_rf_perfect", "pi_brf", "opt_N", "pi_opt"))
  #'   result_all <- rbind(result_all, new_entry)
  #' }
  #' return(result_all)
}
sim_homo <- function (i_sim, field, field_col, exp_design, coef_data, correlation_rho){
  ...
  if (field_col==576){
    ...
    data <- data.table(field) %>%
      ...
      .[, b2:=coef_data$b2] %>%
      .[, Nk:=coef_data$Nk] %>%
      .[, plateau:=coef_data$plateau]%>%
      .[, m_error:=coef_data$m_error] %>%
      .[, N_error:=coef_data$N_error] 
  }else{
  ...
  N_levels <- if (exp_design$num_treatment==4) {
