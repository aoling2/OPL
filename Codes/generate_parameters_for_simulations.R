...
cell_field <- readRDS(here("Code/Data", "cell_field.rds"))

for (i in 1: nrow(cell_field)){

  temp_field <- cell_field[i, ]
  
  field <- readRDS(paste0("Code/Data/field_col_", temp_field$cell_columns_in_field, ".rds"))
  ...
  source(here("./Code/Codes/Functions/parameter_generation.R"))
  object.size(par_data)
  saveRDS(
    par_data,
    here(
      "Data", 
